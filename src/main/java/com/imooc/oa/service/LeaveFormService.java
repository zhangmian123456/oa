...
public class LeaveFormService {
    ...
    public void audit(Long formId, Long operatorId, String result, String reason){
        MyBatisUtils.executeUpdate(sqlSession -> {
            ...
            //获取当前任务ProcessFlow对象
            List<ProcessFlow> processList = flowList.stream().filter(p-> p.getOperatorId().equals(operatorId) && p.getState().equals("process")).collect(Collectors.toList());
            ...
            if(process.getIsLast()==1){
                ...
                //3.如果当前任务不是最后一个节点且审批通过，那下一个节点的状态从ready变更为process
                if(result.equals("approved")){
                    ...
                    //4.如果当前任务不是最后一个节点且审批驳回，则后续所有的任务状态变为cancel，请假单状态变更为refused
